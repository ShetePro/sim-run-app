...
export const useCloudSyncStore = create<CloudSyncState>((set, get) => ({
  ...
  initialize: async () => {
    try {
      // 加载设置
      const storedSettings = (await getStorageItemAsync(SYNC_SETTINGS_KEY)) as
        | string
        | null;
      ...
      // 加载元数据
      const storedMetadata = (await getStorageItemAsync(SYNC_METADATA_KEY)) as
        | string
        | null;
...
export async function shouldAutoSync(): Promise<boolean> {
  const settings = (await getStorageItemAsync(SYNC_SETTINGS_KEY)) as
    | string
    | null;
