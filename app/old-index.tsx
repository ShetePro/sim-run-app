...
export default function HomeScreen() {
  ...
  const [runData, setRunData] = useState<RunRecord[]>([]);
  useEffect(() => {
    getRuns().then((runs) => {
      console.log(runs, "获取");
      setRunData(runs)
  ...
  function totalItem(item: any) {
    return (
      <View
        className={
          "flex flex-col items-center flex-1 bg-white dark:bg-slate-800 rounded-3xl p-3"
        }
        key={item.label}
      >
  ...
  function activityItem(item: any) {
    return (
      <View
        className={"flex flex-col flex-1 bg-white dark:bg-slate-800 rounded-3xl p-4 gap-1"}
        key={item.label}
      >
        <ThemedText style={{ fontSize: 14 }}>{item.label}</ThemedText>
        <ThemedText style={{ fontSize: 24, lineHeight: 40 }}>
