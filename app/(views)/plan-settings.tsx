...
export default function PlanSettingsScreen() {
  ...
  const getPresetMatch = () => {
    if (plan.dailyDistance === 3 && plan.weeklyDistance === 15) return "beginner";
    if (plan.dailyDistance === 5 && plan.weeklyDistance === 20) return "regular";
    if (plan.dailyDistance === 10 && plan.weeklyDistance === 40) return "advanced";
    return "custom";
  };
                  ...
                  <View className="px-4 py-4 border-b border-gray-100 dark:border-slate-700">
                    <View className="flex-row justify-between items-center">
                      ...
                      <NumberStepper
                        ...
                        onChange={(val) => updateSetting("plan.dailyDistance", val)}
                        step={0.5}
                        min={0.5}
                        max={50}
                        unit="km"
                  ...
                  <View className="px-4 py-4 border-b border-gray-100 dark:border-slate-700">
                    <View className="flex-row justify-between items-center">
                      ...
                      <NumberStepper
                        ...
                        onChange={(val) => updateSetting("plan.weeklyDistance", val)}
                        step={5}
                        min={1}
                        max={200}
                        unit="km"
                  ...
                  <View className="px-4 py-4 border-b border-gray-100 dark:border-slate-700">
                    <View className="flex-row justify-between items-center">
                      ...
                      <NumberStepper
                        ...
                        onChange={(val) => updateSetting("plan.weeklyRuns", val)}
                        step={1}
                        min={1}
                        max={14}
                        unit={t("plan.times")}
                  ...
                  <View className="px-4 py-4">
                    <View className="flex-row justify-between items-center">
                      ...
                      <NumberStepper
                        ...
                        onChange={(val) => updateSetting("plan.monthlyDistance", val)}
                        step={10}
                        min={5}
                        max={500}
                        unit="km"
                  ...
                  <View className="px-4 py-3 border-t border-gray-100 dark:border-slate-700">
                    <View className="flex-row justify-between items-center">
                      ...
                      <View className="flex-row items-center bg-gray-100 dark:bg-slate-700 rounded-lg px-3 py-2">
                        <Ionicons name="time-outline" size={16} color="#64748B" className="mr-2" />
                        <TextInput
                          value={plan.reminderTime}
                          onChangeText={(text) => {
                            // 简单的时间格式化
                            const clean = text.replace(/[^0-9:]/g, "");
                            updateSetting("plan.reminderTime", clean.slice(0, 5));
                          }}
                          placeholder="07:00"
                          maxLength={5}
